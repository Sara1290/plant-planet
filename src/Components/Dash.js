    ...
    return (
      <div key={recipes.plant_recipes_id} className="recipe-container">
        ...
        {recipes.author_username === username ? (
          <button onClick={() => deleteRecipe(recipes.plant_recipes_id)}>
            Delete Your Recipe
          </button>
        ) : null}
        <button
          className="add-button"
          onClick={() => addToRecipeBox(recipes.plant_recipes_id)}
        >
          {
            <img
              alt=""
              className="add-button-img"
              src="https://www.clipartkey.com/mpngs/m/50-505406_plus-sign-icon-button-green-approved-check-add.png"
            />
          }
        </button>
